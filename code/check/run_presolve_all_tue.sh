...
while read filename; do
    ...
    then
        ...
        echo "sbatch run_presolve_tue.sh $filename $SETTINGS $TIMELIMIT $SEED $OUTPUTNAME"
        sbatch run_presolve_tue.sh $filename $SETTINGS $TIMELIMIT $SEED $OUTPUTNAME
        # ./run_presolve_tue.sh $filename $SETTINGS $TIMELIMIT $SEED $OUTPUTNAME
    fi
