...
#include <scip/../symmetry/compute_symmetry.h>
...
#define DEFAULT_USESYMRETOPES        TRUE    /**< Whether symretopes should be used instead of symresacks (for non-orbisacks). */
#define DEFAULT_RELABELSYMRETOPES       1    /**< How relabeling should be applied for symretopes. Follows SCIP_SYMRETOPERELABEL (0 = No relabeling, >0 = Some sorted order) */
#define DEFAULT_EXTENDGENERATORS    FALSE    /**< Whether the set of generators should be extended */
#define DEFAULT_MAXEXTENDGENERATORS 134217728 /**< Maximal number of generators in the generating set during extension phase */
#define DEFAULT_ADDMORESYMRESACKS   FALSE    /**< Whether a symresack constraint should only be added for the group generator, or for multiple group elements */
#define DEFAULT_MORESYMRESACKSMAXNPERMS  10000    /**< Maximum number of permutations to consider in a symmetry group when adding more symresacks */
#define DEFAULT_MORESYMRESACKSMAXPRODUCTNPERMSNVARS 5000000 /**< Maximum of permutations and number of variables in the permutation to consider in a symmetry group when adding more symresacks */
...
struct SCIP_PropData
{
   ...
   int                   maxngenorbconss;    /**< array size of genorbconss */
   ...
   SCIP_Bool             usesymretopes;      /**< Whether symretopes should be used for non-orbisack groups. */
   int                   relabelsymretopes;  /**< Whether variable relabeling should be applied. */
   SCIP_Bool             extendgenerators;   /**< Whether generating set should be extended */
   int                   maxextendgenerators;/**< Maximal number of generators in the generating set during extension phase */
   SCIP_Bool             moresymresacks;     /**< Whether we want to add more symresacks */
   int                   moresymresacksmaxnperms; /**< Maximum number of permutations to consider in a symmetry group when adding more symresacks */
   int                   moresymresacksmaxproductnpermsnvars; /**< Maximum of permutations and number of variables in the permutation to consider in a symmetry group when adding more symresacks */
...
static
SCIP_DECL_SORTINDCOMP(sortByIntValue)
{
   /* @todo move to misc.c? */
   if ( ind1 < ind2 )
      return -1;
...
/** checks whether two arrays that are sorted with the same comparator have a common element */
static
SCIP_Bool checkSortedArraysIntHaveOverlappingEntryNulTermSymbol(
   int*                  arr1,               /**< first array */
   int                   narr1,              /**< number of elements in first array */
   int*                  arr2,               /**< second array */
   int                   narr2,              /**< number of elements in second array */
   int                   term,               /**< the terminating symbol */
   SCIP_DECL_SORTINDCOMP((*compfunc))        /**< comparator function that was used to sort arri and arrj; must define a total ordering */
)
{
   /* @todo move to misc.c? */
   int it1;
   ...
   while ( TRUE )  /*lint !e716*/
   {
      cmp = compfunc(NULL, arr1[it1], arr2[it2]);
      if ( cmp < 0 )
      {
         /* comparison function determines arr1[it1] < arr2[it2]
          * increase iterator for arr1
          */
         if ( ++it1 >= narr1 || arr1[it1] == term )
            break;
         continue;
      }
      else if ( cmp > 0 )
      ...
      else
      {
         /* the entries arr1[it1] and arr2[it2] are the same with respect to the comparison function */
         assert( cmp == 0 );
         assert( arr1[it1] != term );
         assert( arr2[it2] != term );
         return TRUE;
      }
   }

   /* no overlap detected */
   assert( it1 >= narr1 || it2 >= narr2 || arr1[it1] == term || arr2[it2] == term );
   return FALSE;
}
   ...
   )
{
   ...
   if ( propdata->genorbconss != NULL )
   {
      ...
      SCIPfreeBlockMemoryArray(scip, &propdata->genorbconss, propdata->maxngenorbconss);
   ...
   )
{
   ...
   {
      SCIPfreeBlockMemoryArrayNull(scip, &propdata->genorbconss, propdata->maxngenorbconss);
   }
   else
   {
      assert( propdata->genorbconss != NULL );
   ...
   )
{
   ...
   *nusedperms = 0;
   ...
   for (j = 0; j < ntwocycleperms; ++j)
   {
      ...
      {
         int newsize;
         newsize = SCIPcalcMemGrowSize(scip, (*nusedperms) + 1);
...
{  /*lint --e{571}*/
   ...
   *success = TRUE;
   ...
   if ( propdata->ngenorbconss >= propdata->maxngenorbconss )
   {
      int newsize;
      newsize = SCIPcalcMemGrowSize(scip, propdata->ngenorbconss + 1);
      assert( newsize >= 0 );
      assert( newsize > propdata->ngenorbconss );
      SCIP_CALL( SCIPreallocBlockMemoryArray(scip, &(propdata->genorbconss), propdata->maxngenorbconss, newsize) );
      propdata->maxngenorbconss = newsize;
   }
...
/* Sorting function for symretope sorting */
struct symretopeSortPermutationsData
{
   int* permsmaxcyclesize;
   int* permsorder;
};

static
SCIP_DECL_SORTINDCOMP(symretopeSortPermutations)
{
   struct symretopeSortPermutationsData* data;
   data = dataptr;

   if ( data->permsmaxcyclesize[ind1] < data->permsmaxcyclesize[ind2] )
      return 1;
   else if ( data->permsmaxcyclesize[ind1] > data->permsmaxcyclesize[ind2] )
      return -1;
   else
   {
      if ( data->permsorder[ind1] < data->permsorder[ind2] )
         return 1;
      else if ( data->permsorder[ind1] > data->permsorder[ind2] )
         return -1;
   }
   return 0;
}
...
static
SCIP_DECL_SORTINDCOMP(symretopeSortPermutationDecreasingCycleSize)
{
   int* data;
   data = dataptr;
   ...
   if ( data[ind1] < data[ind2] )
      return 1;
   else if ( data[ind1] > data[ind2] )
      return -1;
   return 0;
}

static
SCIP_DECL_SORTINDCOMP(symretopeSortPermutationIncreasingCycleSize)
{
   int* data;
   data = dataptr;
...
static
SCIP_DECL_SORTINDCOMP(symretopeSortPermutationIncreasingSubcycleMinIndex)
{
   int** cycles;
   cycles = dataptr;

   if ( cycles[ind1][0] > cycles[ind2][0] )
      return 1;
   else if ( cycles[ind1][0] < cycles[ind2][0] )
      return -1;
   return 0;
}

/* The symretope propagator can compute the complete set of fixings for cyclic groups that are monotone and ordered.
 * I.e. the generating permutation is monotone (having exactly 1 descend point per disjoint cycle),
 * and the generating permutation is ordered (the disjoint cycles of this permutation are ordered).
 * This function relabels the permutations such that (we hope) to find many monotone and ordered generators.
 *
 * Heuristic principles:
 * 1. We expect most fixings if large cycles have the smallest indices: So large disjoint cycles go on front.
 * 2. We expect more fixings if the order of the generating permutation is large.
 *
 * So we do:
 * 1. Sort permutations (1) by decreasing maximal cycle-length, and (2, tie-breaking) by generator order.
 * 2. Maintain partial ordering of indices (initialized empty). This is a dense graph with an arc (i, j) if i < j.
 *    Iteratively: Try to add permutation as monotone and ordered, respecting the partial ordering.
 *
 * Note: We do this in an heuristic manner, and the relabeling that we find is highly dependent on the order in which
 * the permutations are processed.
 */
static
SCIP_RETCODE adaptSymmetryDataSymretope(
   SCIP*                 scip,               /**< SCIP instance */
   int                   relabelsymretopes,  /**< variant of variable relabeling should be applied, from SCIP_RELABEL */
   int**                 perms,              /**< permutation matrix w.r.t. original variable ordering */
   int                   nperms,             /**< number of permutations */
   int                   nvars,              /**< length or modifiedpermvars array */
   int*                  topoorder           /**< where to store the new ordering. */
)
{
   int i;
   int j;
   ...
   int n;
   #ifndef NDEBUG
   int npermsorderedmonotone = 0;
   #endif
   int ind;
   int* perm;
   int* permsmaxcyclesize;
   int* permsorder;
   struct symretopeSortPermutationsData* sortingdata;
   int* sortedpermsindices;
   int* checked;
   int order;
   int maxcyclesize;
   int thiscyclesize;
   ...
   int* sortedcycles;
   int* cycle;
   SCIP_Bool* iscontainedinpartialorder;
   SCIP_Bool orderingpossible;
   int* partialorder;

   /* If we are not permitted to relabel, return the trivial relabeling. */
   if ( relabelsymretopes == SCIP_RELABEL_NONE )
   {
      for (i = 0; i < nvars; ++i)
         topoorder[i] = i;
      return SCIP_OKAY;
   ...
   /* Relabeling should be applied. */
   SCIP_CALL( SCIPallocBufferArray(scip, &sortedpermsindices, nperms) );
   SCIP_CALL( SCIPallocBuffer(scip, &permutation) );

   /* Allocation for sorting permutations */
   SCIP_CALL( SCIPallocBufferArray(scip, &permsmaxcyclesize, nperms) );
   SCIP_CALL( SCIPallocBufferArray(scip, &permsorder, nperms) );
   SCIP_CALL( SCIPallocBuffer(scip, &sortingdata) );

   /* For every permutation: compute the order and maximal cycle length */
   ...
   for (k = 0; k < nperms; ++k)
   {
      ...
      for (i = 0; i < nvars; ++i)
      {
         ...
         maxcyclesize = MAX(maxcyclesize, thiscyclesize);
      }

      /* Store the maximal cycle length and the permutation order */
      permsmaxcyclesize[k] = maxcyclesize;
      permsorder[k] = order;
   ...
   /* Sort the permutations (1) by decreasing maximal cycle-length, and (2, tie-breaking) by generator order. */
   for (k = 0; k < nperms; ++k)
      sortedpermsindices[k] = k;
   sortingdata->permsmaxcyclesize = permsmaxcyclesize;
   sortingdata->permsorder = permsorder;
   SCIPsort(sortedpermsindices, symretopeSortPermutations, (void*) sortingdata, nperms);
   SCIPfreeBuffer(scip, &sortingdata);
   SCIPfreeBufferArray(scip, &permsorder);
   SCIPfreeBufferArray(scip, &permsmaxcyclesize);

   /* Make the partial ordering. */
   SCIP_CALL( SCIPallocBufferArray(scip, &partialorder, nvars) );
   SCIP_CALL( SCIPallocClearBufferArray(scip, &iscontainedinpartialorder, nvars) );

   /* An array in which we sort the cycle indices.
    * This array needs at most the number of cycles of a permutation. We allocate nvars, which is plenty of space. */
   SCIP_CALL( SCIPallocBufferArray(scip, &sortedcycles, nvars) );

   n = 0;
   for (k_ = 0; k_ < nperms; ++k_)
   {
      k = sortedpermsindices[0];
      perm = perms[k];
      SCIP_CALL( SCIPgetPermutation(scip, perm, nvars, permutation) );

      /* Make sure we can fit the cycle indices in sortedcycles, which has length nvars. */
      assert(permutation->ncycles <= nvars);

      /* Sort the cycles by the specified sorted order */
      switch (relabelsymretopes)
      {
         case SCIP_RELABEL_DECREASINGCYCLESIZE:
            /* Sort cycles to be decreasing in cycle size. */
            SCIPsort(sortedcycles, symretopeSortPermutationDecreasingCycleSize,
               (void*) permutation->cyclelengths, permutation->ncycles);
            break;
         case SCIP_RELABEL_INCREASINGCYCLESIZE:
            /* Sort cycles to be increasing in cycle size. */
            SCIPsort(sortedcycles, symretopeSortPermutationIncreasingCycleSize,
               (void*) permutation->cyclelengths, permutation->ncycles);
            break;
         case SCIP_RELABEL_INCREASINGSUBCYCLEMININDEX:
            /* Sort subcycles by smallest original index in the subcycles.
             * Note: The smallest original index of a cycle is always at the first position of the cycle,
             * by the way the permutations are constructed.
             */
            SCIPsort(sortedcycles, symretopeSortPermutationIncreasingSubcycleMinIndex,
               (void*) permutation->cycles, permutation->ncycles);
            break;
         default:
      ...
      /* Check if it is possible to add this permutation to the ordering. */
      orderingpossible = TRUE;
      for (c_ = 0; c_ < permutation->ncycles; ++c_)
      {
         c = sortedcycles[c_];
         ...
         /* If there is a non-fixed entry that is already ordered, then we ignore this permutation. */
         for (i = 0; i < cyclen; ++i)
         {
            /* As part of the heuristic: We are interested in a set of disjoint permutations in the component.
             * So, even though the labeling of two overlapping permutations may be compatable, we only relabel at most
             * one of two overlapping permutations.
             * A previous implementation encoded the partial ordering with a transitively closed graph representing the
             * partial ordering, and added permutations one-by-one. This ensured that compatable permutations are both
             * added. However, this is too computationally intensive, so we settle at adding non-overlapping
             * permutations.
             */
            if ( iscontainedinpartialorder[cycle[i]] )
            {
               orderingpossible = FALSE;
               break;
            }
         }
      }

      /* If this permutation is not possible, then try the next permutation. */
      if ( !orderingpossible )
         goto CleanIteration;
      ...
      for (c_ = 0; c_ < permutation->ncycles; ++c_)
      {
         ...
         for (i = 0; i < cyclen; ++i)
         {
            ind = cycle[i];
            partialorder[n++] = ind;
            assert( !iscontainedinpartialorder[ind] );
            iscontainedinpartialorder[ind] = TRUE;
         }
      }
      assert( n <= nvars );

      #ifndef NDEBUG
      ++npermsorderedmonotone;
      #endif

      CleanIteration:
      SCIP_CALL( SCIPfreePermutationContents(scip, permutation, FALSE) );
   }
   SCIPfreeBufferArray(scip, &sortedcycles);
   assert( n <= nvars );

   /* We can always make at least one permutation monotone and ordered. */
   assert( nperms <= 0 || npermsorderedmonotone > 0 );

   /* Now fix the topological ordering */
   m = 0;
   for (k = 0; k < nvars; ++k)
   {
      if ( iscontainedinpartialorder[k] )
         topoorder[k] = partialorder[m++];
      else
         topoorder[k] = k;
   }
   assert( n == m );
   ...
   /* Free memory */
   SCIPfreeBufferArray(scip, &iscontainedinpartialorder);
   SCIPfreeBufferArray(scip, &partialorder);
   SCIPfreeBuffer(scip, &permutation);
   SCIPfreeBufferArray(scip, &sortedpermsindices);

   return SCIP_OKAY;
}


/* given a set of permutations and the number hereof (as a pointer value),
 * update this set of permutations with additional permutations.
 */
static
SCIP_RETCODE adaptSymmetryExtendGenerators(
   SCIP*                 scip,               /**< SCIP instance */
   int***                perms,              /**< pointer to permutation matrix w.r.t. original variable ordering */
   int*                  nperms,             /**< pointer to number of permutations */
   int                   npermvars,          /**< number of variables in the permutations */
   int                   maxnperms           /**< maximal number of elements in 'perms' */
)
{
   int i;
   int j;
   int k;
   SCIP_PERMUTATION* initperms;
   int ninitperms;
   int* initpermsupports;
   int permsmaxarraysize;

   int* thisinitpermsupport;
   ...
   /* get the initial permutations as a permutation-object. */
   ninitperms = *nperms;
   SCIP_CALL( SCIPallocBufferArray(scip, &initperms, ninitperms) );
   for (k = 0; k < ninitperms; ++k)
      SCIP_CALL( SCIPgetPermutation(scip, (*perms)[k], npermvars, &initperms[k]) );

   /* for each permutation, get the indices in the support */
   SCIP_CALL( SCIPallocBufferArray(scip, &initpermsupports, ninitperms * npermvars) );
   for (k = 0; k < ninitperms; ++k)
   {
      perm = &initperms[k];
      thisinitpermsupport = &initpermsupports[k * npermvars];

      j = 0; /* the index of thisinitpermsupport */
      /* if the cyclelength containing index i is at least 1, then it's in the support of the permutation perm */
      for (i = 0; i < npermvars; ++i)
      {
         if ( perm->cyclelengths[perm->varcycle[i]] > 1 )
            thisinitpermsupport[j++] = i;
      }
      /* mark remaining entries of support as -1 */
      for (; j < npermvars; ++j)
         thisinitpermsupport[j] = -1;
   }

   /* we re-allocate the perms array regularly, so keep track of the maximal size */
   permsmaxarraysize = *nperms;

   /* for each pair of non-overlapping indices, compute the product, and add to the list perms of permutations */
   for (i = 0; i < ninitperms; ++i)
   {
      /* stop if we've reached the maximally desired number of permutations */
      if ( *nperms >= maxnperms )
         break;

      for (j = i + 1; j < ninitperms; ++j)
      {
         /* stop if we've reached the maximally desired number of permutations */
         if ( *nperms >= maxnperms )
            break;

         /* only compute the product if permutations i and j overlap */
         if ( ! checkSortedArraysIntHaveOverlappingEntryNulTermSymbol(
            &initpermsupports[i * npermvars], npermvars,
            &initpermsupports[j * npermvars], npermvars, -1, sortByIntValue)
         )
            continue;

         /* compute product, and insert to perms array. */
         SCIP_CALL( SCIPallocBlockMemoryArray(scip, &permarr, npermvars) );  /* Make sure that this gets freed!! */

         for (k = 0; k < npermvars; ++k)
         {
            /* compute image of k on gamma * delta, where gamma is (*perms)[i] and delta is (*perms)[j] */
            permarr[k] = ((*perms)[i])[((*perms)[j])[k]];
         }

         /* check whether we need to resize */
         if ( *nperms >= permsmaxarraysize )
         {
            int newsize;

            newsize = SCIPcalcMemGrowSize(scip, *nperms + 1);
            assert( newsize > *nperms );

            SCIP_CALL( SCIPreallocBlockMemoryArray(scip, perms, permsmaxarraysize, newsize) );

            permsmaxarraysize = newsize;
         }

         /* Insert permarr to perms array */
         (*perms)[(*nperms)++] = permarr;
      }
   }

   /* make sure that the array sizes match, so shrink the array */
   if ( permsmaxarraysize != *nperms )
   {
      assert( permsmaxarraysize > *nperms );
      SCIP_CALL( SCIPreallocBlockMemoryArray(scip, perms, permsmaxarraysize, *nperms) );
      permsmaxarraysize = *nperms;
   }

   /* remove everything we need */
   SCIPfreeBufferArray(scip, &initpermsupports);
   for (k = ninitperms - 1; k >= 0; --k)
      SCIP_CALL( SCIPfreePermutationContents(scip, &initperms[k], FALSE) );
   assert( k == -1 );
   SCIPfreeBufferArray(scip, &initperms);
 ...
 * if the found orbitopes do not satisfor each pair of permutations, store the y certain criteria for being used
 */
static
...
/** Add the symresack or symretope symmetry breaking constraints, dependent on the problem settings. */
static
SCIP_RETCODE addSymmetryBreakingConstraintSymretopeOrSymresack(
   SCIP* scip,                               /**< The SCIP instance */
   SCIP_PROPDATA* propdata,                  /**< The propagation data */
   int* perm,                                /**< A permutation of the entries 0..nvars-1, in array form */
   SCIP_VAR** vars,                          /**< The list of variables (nvars many) */
   int nvars,                                /**< The number of variables affected by the permutation */
   int compid,                               /**< The component ID of non-overlapping permutations, if applicable. */
   int permid                                /**< The permutation ID within the component, if applicable. */
)
{
   assert( scip != NULL );
   assert( propdata != NULL );
   ...
   if ( propdata->usesymretopes )
   {
      SCIP_CONS* cons;
      char name[SCIP_MAXSTRLEN];

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "symretope_comp%d_perm%d", compid, permid);

      SCIP_CALL( SCIPcreateSymbreakConsSymretope(scip, &cons, name,
         perm, vars, nvars, FALSE,
   ...
   else if ( propdata->moresymresacks )
   {
      SCIP_PERMUTATION* permutation;
      int* permarr;
      int nperms;
      ...
      nperms = MIN(permutation->order - 1, INT_MAX);
      nsupport = 0;
      for (ind = 0; ind < propdata->npermvars; ++ind)
      {
         if ( perm[ind] != ind )
            ++nsupport;
      }

      if ( nperms > propdata->moresymresacksmaxnperms
         || nperms * nsupport > propdata->moresymresacksmaxproductnpermsnvars )
         SCIPwarningMessage(scip, "Symretope constraint will not capture all symmetries.\n");
      if ( nperms > propdata->moresymresacksmaxnperms )
      {
         nperms = propdata->moresymresacksmaxnperms;
         SCIPwarningMessage(scip, "=> The symmetry group order %lld is larger than maxorder: %d. "
            "Restricting to %d permutations.\n",
            permutation->order, propdata->moresymresacksmaxnperms, nperms);
      }
      if ( nperms * nsupport > propdata->moresymresacksmaxproductnpermsnvars )
      {
         nperms = propdata->moresymresacksmaxproductnpermsnvars / nsupport;
         ...
         SCIPwarningMessage(scip, "=> The symmetry group order * cardinality of support (%lld * %d) "
            ...
            permutation->order, nsupport, propdata->moresymresacksmaxproductnpermsnvars, nperms);
      }

      SCIP_CALL( SCIPallocBufferArray(scip, &permarr, propdata->npermvars) );
      ...
      for (j = 1; j <= nperms; ++j)
      {
         ...
         (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "symresack_comp%d_perm%d_%d", compid, permid, j);

         SCIP_CALL( getPermArray(permutation, j, permarr, propdata->npermvars) );

         SCIP_CALL( SCIPcreateSymbreakCons(scip, &cons, name,
            permarr, vars, nvars, FALSE,
            propdata->conssaddlp, TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
         ...
         if ( propdata->ngenorbconss >= propdata->maxngenorbconss )
         {
            ...
            assert( newsize >= 0 );
            assert( newsize > propdata->ngenorbconss );
            SCIP_CALL( SCIPreallocBlockMemoryArray(scip, &(propdata->genorbconss),
               propdata->maxngenorbconss, newsize) );
            propdata->maxngenorbconss = newsize;
         }
         propdata->genorbconss[propdata->ngenorbconss++] = cons;
      ...
      SCIPfreeBufferArray(scip, &permarr);
      SCIPfreePermutationContents(scip, permutation, FALSE);
      SCIPfreeBuffer(scip, &permutation);
   }
   else
   {
      ...
      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "symresack_comp%d_perm%d", compid, permid);
   ...
   if ( compid >= 0 && ! propdata->componentblocked[compid] )
   {
      assert( compid < propdata->ncomponents );
      propdata->componentblocked[compid] |= SYM_HANDLETYPE_SYMBREAK;
   ...
   )
{
   ...
   for (i = 0; i < propdata->ncomponents; ++i)
   {
      ...
      if ( norbitopesincomp == 1 )
      {
         ...
         /* List the permutations in this component. */
         SCIP_CALL( SCIPallocBufferArray(scip, &permsincomp, npermsincomp) );
         for (k = 0; k < npermsincomp; ++k)
            permsincomp[k] = propdata->perms[propdata->components[propdata->componentbegins[i] + k]];

         /*
          * The following part of the code is wrong!
          * Here, We cannot relabel here, because we should stick to the orbitope labelling!
          */
         assert( FALSE );

         /* Relabel (This is only nontrivial for symretopes if relabeling is permitted) */
         SCIP_CALL( SCIPallocBufferArray(scip, &topoorder, propdata->npermvars) );
         SCIP_CALL( adaptSymmetryDataSymretope(scip, propdata->relabelsymretopes, permsincomp, npermsincomp,
               propdata->npermvars, topoorder) );
         SCIP_CALL( adaptSymmetryDataSST(scip, propdata->perms, modifiedperms, propdata->nperms,
               propdata->permvars, modifiedpermvars, propdata->npermvars, topoorder, propdata->npermvars) );
         #ifndef NDEBUG
         {
            /* Test: modifiedperms now follows the variable ordering of topoorder. */
            for (k = 0; k < propdata->npermvars; ++k)
            {
               assert( propdata->permvars[topoorder[k]] == modifiedpermvars[k] );
            }
         }
         #endif
         ...
         for (k = 0; k < npermsincomp; ++k)
         {
            SCIP_CALL( addSymmetryBreakingConstraintSymretopeOrSymresack(scip, propdata,
               modifiedperms[propdata->components[propdata->componentbegins[i] + k]],
               modifiedpermvars, propdata->npermvars, i, k) );
            ...
            SCIPdebugMsg(scip, "  add symresack/symretope for permutation %d of component %d\n", k, i);
         }

         SCIPfreeBufferArray(scip, &permsincomp);
         goto FREELOOPMEMORY;
      }
...
{ /*lint --e{641}*/
   ...
   /* We always compute components, so there should be at least one. */
   ...
   if ( ncomponents == -1 )
   {
      ...
      assert( !( propdata->addsymresacks && propdata->relabelsymretopes ) );

      /* loop through perms and add symresack constraints */
      for (p = 0; p < propdata->nperms; ++p)
      {
         SCIP_CALL( addSymmetryBreakingConstraintSymretopeOrSymresack(scip, propdata,
            perms[p], permvars, npermvars, -1, p) );
   ...
   else
   {
      ...
      for (i = 0; i < ncomponents; ++i)
      {
         ...
         /* If SST constraints determined a variable ordering, cannot relabel. */
         if ( propdata->nleaders > 0 && ISSSTBINACTIVE(propdata->sstleadervartype) )
         {
            ...
            for (p = componentbegins[i]; p < componentbegins[i + 1]; ++p)
            {
               ...
               SCIP_CALL( addSymmetryBreakingConstraintSymretopeOrSymresack(scip, propdata,
                  modifiedperms[permidx], modifiedpermvars, npermvars, i, permidx) );

               propdata->componentblocked[i] |= SYM_HANDLETYPE_SYMBREAK;
               ++nsymresackcons;
            }
         }
         /* Otherwise, if there are no SST constraints, then we can determine a labeling! */
         else
         {
            int** permsincomp;
            int* topoorder;
            ...
            /* @todo the original array modifiedperms is not suitable here, because it assumes that we have exactly as
             * many permutations as in propdata->perms. That is now wrong. Instead, we temporarily introduce this
             * modified permutation list and handle the component with this. SST-cuts are not relevant here,
             * so that's good. In principle, SST cuts handle symmetries within a component anyway, so the leader-order
             * also needs to be specified per component.
             *
             * I believe the split of components should be more explicit in the code. This way, we do not need to keep
             * track of variables in other components. As described:
             *
             *
             * In the future, I think the code that splits up the symmetry group components in prop_symmetry should be
             * more explicit.
             *
             * Right now, SST cuts are not added per-component, while they are effectively handled per-component.
             * Because these SST-cuts 'hover' above the split of components, we need to be careful in the code that we
             * don't mix up different components.
             *
             * Especially that part of code complicates the code for scip-symretope. Because we've disabled SST anyway,
             * I'm planning [red; That is the code below here] to 'hack' it for the new symretope code, which will mean * that this part is not correct if SST cuts are being added for real.
             *
             * The other thing is that all variables in all components are pushed though all functions, while the
             * symmetry handling should only be applied on the variables within the component.
             *
             * So I would suggest to do the following:
             *
             * 1. Compute the components, and  for each component, make a separate array variables and make the
             *    permutations of the component compatible with that variable labeling
             * 2. A function that accepts a component (component index (for logging only), a variable array containing
             *    only the variables in that component, and the permutations indexed by the positions in that variable
             *    array), and does the things which we know:
             *    2.1. Full orbitope detection
             *    2.2. Subgroup detection
             *    2.3. SST on its own component (I don't care if the order if leaders is [var from component 1, var
             *    from component 1, var of component 2] instead of [var from component 1, var from component 2, var
             *    from component 1], as long as the order within a component remains consistent.
             *    2.4. Symresack/symretope/etc
             *
             * The main complication in the current code, is that 'inserting new permutations' within a component is
             * quite complicated, because the permutation-array sorted the permutations from componentbegins[i] to
             * componentbegins[i+1]-1. I shouldn't touch that array.
             */
            int** modifiedpermscomp;
            SCIP_VAR** modifiedpermvarscomp;

            /* List the permutations in this component. */
            npermsincomp = componentbegins[i + 1] - componentbegins[i];
            SCIP_CALL( SCIPallocBlockMemoryArray(scip, &permsincomp, npermsincomp) );
            for (p = 0; p < npermsincomp; ++p)
               permsincomp[p] = propdata->perms[propdata->components[propdata->componentbegins[i] + p]];

            /* Possibly extend the permutations in this component */
            if ( propdata->extendgenerators )
            {
               SCIP_CALL( adaptSymmetryExtendGenerators(scip, &permsincomp, &npermsincomp, propdata->npermvars,
                  propdata->maxextendgenerators / propdata->npermvars) );
            }

            SCIP_CALL( SCIPallocBufferArray(scip, &modifiedpermscomp, npermsincomp) );
            SCIP_CALL( SCIPallocBufferArray(scip, &modifiedpermvarscomp, propdata->npermvars) );
            for (k = 0; k < npermsincomp; ++k)
            {
               SCIP_CALL( SCIPallocBufferArray(scip, &modifiedpermscomp[k], propdata->npermvars) );
            }
            ...
            SCIP_CALL( adaptSymmetryDataSymretope(scip, propdata->relabelsymretopes, permsincomp, npermsincomp,
                  propdata->npermvars, topoorder) );
            SCIP_CALL( adaptSymmetryDataSST(scip, permsincomp, modifiedpermscomp, npermsincomp,
                  propdata->permvars, modifiedpermvarscomp, propdata->npermvars, topoorder, propdata->npermvars) );
            #ifndef NDEBUG
            {
               /* Test: modifiedpermscomp now follows the variable ordering of topoorder. */
               for (k = 0; k < propdata->npermvars; ++k)
               {
                  assert( propdata->permvars[topoorder[k]] == modifiedpermvarscomp[k] );
               }
            }
            #endif
            SCIPfreeBufferArray(scip, &topoorder);
            ...
            {
               SCIP_CALL( addSymmetryBreakingConstraintSymretopeOrSymresack(scip, propdata,
                  modifiedpermscomp[k], modifiedpermvarscomp, propdata->npermvars, i, k) );
            ...
            /* deallocate any newly generated permutations after extension */
            if ( propdata->extendgenerators )
            {
               /* in adaptSymmetryExtendGenerators, we allocated new perms. Here, free these newly generated ones */
               k = npermsincomp;
               while (k > componentbegins[i + 1] - componentbegins[i])
               {
                  SCIPfreeBlockMemoryArray(scip, &(permsincomp[--k]), propdata->npermvars);
               }
               /* we must be back to the initial npermsincomp-value */
               assert( k == componentbegins[i + 1] - componentbegins[i] );
            }

            for (k = npermsincomp - 1; k >= 0; --k)
            {
               SCIPfreeBufferArray(scip, &modifiedpermscomp[k]);
            }
            SCIPfreeBufferArray(scip, &modifiedpermvarscomp);
            SCIPfreeBufferArray(scip, &modifiedpermscomp);

            SCIPfreeBlockMemoryArray(scip, &permsincomp, npermsincomp);
         }
      }
   }

   SCIPdebugMsg(scip, "Added %d (sets of) symresack constraints.\n", nsymresackcons);
   ...
   )
{
   ...
   {
      propdata->maxngenorbconss = propdata->nperms;
      SCIP_CALL( SCIPallocBlockMemoryArray(scip, &propdata->genorbconss, propdata->maxngenorbconss) );
   ...
   )
{
   ...
   SCIP_CALL( SCIPaddBoolParam(scip,
         "propagating/" PROP_NAME "/usesymretopes",
         "Whether symretopes should be used for non-orbisack groups.",
         &propdata->usesymretopes, TRUE, DEFAULT_USESYMRETOPES, NULL, NULL) );

   SCIP_CALL( SCIPaddIntParam(scip,
         "propagating/" PROP_NAME "/relabelsymretopes",
         "If possible, whether variable relabeling should be applied for symretopes, following the SCIP_SYMRETOPERELABEL enum",
         &propdata->relabelsymretopes, TRUE, DEFAULT_RELABELSYMRETOPES, 0, INT_MAX, NULL, NULL) );

   SCIP_CALL( SCIPaddBoolParam(scip,
         "propagating/" PROP_NAME "/extendgenerators",
         "Whether we want to add more symresacks",
         &propdata->extendgenerators, TRUE, DEFAULT_EXTENDGENERATORS, NULL, NULL) );

   SCIP_CALL( SCIPaddIntParam(scip,
         "propagating/" PROP_NAME "/maxextendgenerators",
         "Maximal number of generators in the generating set during extension phase",
         &propdata->maxextendgenerators, TRUE, DEFAULT_MAXEXTENDGENERATORS, 0, INT_MAX, NULL, NULL) );

   SCIP_CALL( SCIPaddBoolParam(scip,
         "propagating/" PROP_NAME "/addmoresymresacks",
         "Whether we want to add more symresacks",
         &propdata->moresymresacks, TRUE, DEFAULT_ADDMORESYMRESACKS, NULL, NULL) );

   SCIP_CALL( SCIPaddIntParam(scip,
         "propagating/" PROP_NAME "/moresymresacksmaxnperms",
         "Maximum number of permutations to consider in a symmetry group when adding more symresacks",
         &propdata->moresymresacksmaxnperms, TRUE, DEFAULT_MORESYMRESACKSMAXNPERMS, 0, INT_MAX, NULL, NULL) );

   SCIP_CALL( SCIPaddIntParam(scip,
         "propagating/" PROP_NAME "/moresymresacksmaxproductnpermsnvars",
         "Maximum of permutations and number of variables in the permutation to consider in a symmetry group when adding more symresacks",
         &propdata->moresymresacksmaxproductnpermsnvars, TRUE, DEFAULT_MORESYMRESACKSMAXPRODUCTNPERMSNVARS, 0, INT_MAX, NULL, NULL) );
